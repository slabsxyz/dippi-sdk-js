import { Client, ApplicationResponseBody, ApplicationCreatePayload, ApplicationUpdatePayload } from '../types/interfaces';
...
class Application {
     ...
     */
    async create (data: ApplicationCreatePayload): Promise<ApplicationResponseBody> {
        const response = await fetch(
            `${this.client.url}/v1/applications`,
            {
                method: 'POST',
     ...
     */
    async update (id: string | number, data: ApplicationUpdatePayload): Promise<ApplicationResponseBody> {
        const response = await fetch(
            `${this.client.url}/v1/applications/${id}`,
            {
                method: 'PATCH',
